...
class Ftdi(object):
    ...
    def read_data_bytes(self, size, attempt=1):
        ...
        #print "Request Length: 0x%02X" % size
        #print "Chunk size: 0x%08X" % self.read_data_get_chunksize()
            ...
            while (len(data) < size) and (length > 0):
                while True:
                    ...
                    if length > 2:
                        ...
                        #print "Only modem status... %d" % length
                        if attempt > 0:
                            continue
                        # no actual data
                        self.readbuffer = Array('B')
                ...
                if length > 0:
                    ...
                    if (len(data) + length) <= size:
                        ...
                        self.readoffset += length
                        #print "data still in buffer: %d" % len(data)
                        ...
                        print "size - length: %d" % (size - len(data))
                        print "Read offset: %d" % self.readoffset
                        '''
                        #self.readoffset + 2
                        '''
                        if (size - len(data)) == 2:

                            print "We have modem bytes"
                            self.readoffset + 2
                            
                        '''
                        #self.readoffset + 2
                        part_size = len(self.readbuffer)-self.readoffset
                        #print "part size: %d" % part_size
                        if part_size < 0:
                            raise FtdiError("Internal Error")

                        #print "Read Buffer:"
                        #print_8bit_hex_array(self.readbuffer)
                        data += self.readbuffer[self.readoffset + 2:
                                                self.readoffset + part_size]
                        #                        self.readoffset + 2 + part_size - 2]

                        '''
                        print "size - len(data): %d" % (size - len(data))
                        print "read buf: %d" % (len(self.readbuffer) - self.readoffset)
                        ...
                        data += self.readbuffer[self.readoffset:
                                                self.readoffset+part_size]
                        self.readoffset += part_size
                        print "not everything: %d" % len(data)
                        print "First two bytes: 0x%02X 0x%02X" % (data[0], data[1])
                        data = data[2:]
                        self.readoffset -= 2
                        print "continue!"
                        '''
